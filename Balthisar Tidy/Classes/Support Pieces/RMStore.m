...
//
//  2019-November diffs added by Jim Derry to eliminate compiler warnings.
...
- (void)didPurchaseTransaction:(SKPaymentTransaction *)transaction queue:(SKPaymentQueue*)queue
{
    ...
    id<RMStoreReceiptVerifier> receiptVerifier = self.receiptVerifier;
...
{
    id<RMStoreContentDownloader> contentDownloader = self.contentDownloader;
    if (contentDownloader != nil)
    {
        [contentDownloader downloadContentForTransaction:transaction success:^{
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:RMSKProductsRequestFinished object:store userInfo:userInfo];
}

- (void)requestDidFinish:(SKProductsRequest *)request
{
    RMStore *store = self.store;
    [store removeProductsRequestDelegate:self];
    [store removeProductsRequest:request];
}
...
- (void)request:(SKProductsRequest *)request didFailWithError:(NSError *)error
{
    ...
    RMStore *store = self.store;
    [[NSNotificationCenter defaultCenter] postNotificationName:RMSKProductsRequestFailed object:store userInfo:userInfo];
    [store removeProductsRequestDelegate:self];
    [store removeProductsRequest:request];
}
